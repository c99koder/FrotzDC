...
/*
 * Dreamcast VMU support added by Sam Steele <chip@c99.org>
 */
...
void z_save (void)
{
  ...
  /* Check if we're saving to a VMU */
  ...
  if(vmu) {
    fs_chdir(tmp);
    sprintf(tmp,"/ram/%s",new_name);
    vmuify(tmp,new_name,"Frotz Save","A FrotzDC Saved Game","FrotzDC");
  }
	/* Success */
